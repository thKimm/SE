...
class DatasetVoiceDemand(torch.utils.data.Dataset):
    def __init__(self,hp,is_train=True):
        ...
        if is_train : 
            ...
            self.list_noisy = glob(join(hp.data.dev.root,"noisy_testset_wav","*.wav"),recursive=True)
            self.list_clean = glob(join(hp.data.dev.root,"clean_testset_wav","*.wav"),recursive=True)
        ...
        if is_train : 
            ...
            print("DatasetVoicebank+Demand[train:{}] | len : {} | noisy : {}".format(is_train,self.n_item,len(self.list_noisy)))
