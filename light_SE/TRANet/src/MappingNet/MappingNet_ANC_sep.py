...
class MappingNet(nn.Module):
    ...
    def forward(self, x, aec):
        if len(x.shape) == 3:
            ...
            X3m = torch.abs(torch.stft(x[:,2,:],
                n_fft = self.frame_size, 
                window=torch.hann_window(self.frame_size).to(x.device),
                return_complex=True)).unsqueeze(dim=-1)
            X = torch.cat([X1,X1m,X2,X2m,X1_AEC,X1m_AEC,X2_AEC,X2m_AEC,X3,X3m],dim=-1)
        else :
            B,L = x.shape
            X = self._to_spec(x)
        # x.shape == (B,F,T,2)
        ...
        Y = self.m_out(Y)
        # Y_sep = nspk * [B,F,T,2]
        Y_sep = torch.chunk(Y, 2, dim=-1)
        Y_sep = [Y_sep[i] for i in range(2)]
        y = [self._to_signal(Y_sep[i])[:, :L] for i in range(2)]
