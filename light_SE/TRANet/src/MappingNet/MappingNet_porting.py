...
class F_ConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, groups = 1):
        ...
        # self.conv = nn.ModuleList([nn.Conv1d(in_channels//groups, out_channels//groups, kernel_size=kernel_size, stride=stride, padding=padding) for _ in range(groups)])
        self.conv = nn.ModuleList([nn.Conv2d(in_channels//groups, out_channels//groups, kernel_size=(kernel_size,kernel_size), stride=(stride,stride), padding=(padding,padding)) for _ in range(groups)])
        self.ln = nn.LayerNorm(out_channels)
        self.act = nn.PReLU(out_channels)
        self.groups = groups
        self.groups_inchannel = in_channels // groups
        
    def forward(self, x):
        '''
            x : [B, C, F]
                => [B, C, F, F]
                FxF 2D : diagnal matrix with diagonal elements as F of x
        '''
        print("="*20+"FConv"+"="*20)
        print("input : ",x.shape)
        x = x.transpose(-1,1)
        print("transpose1 : ",x.shape)
        x = self.ln(x)
        x = x.transpose(-1,1)
        print("transpose2 : ",x.shape)
        x = torch.cat([conv(x[:,i*self.groups_inchannel:(i+1)*self.groups_inchannel,:]) for i, conv in enumerate(self.conv)],dim=1)
        x = self.act(x)
...
class F_FullbandBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dropout:
            ...
            x = x.transpose(1,3)
            print("transpose3 : ",x.shape)
            x = self.dropout(x)
            x = x.transpose(1,3)
            print("transpose4 : ",x.shape)
            x = x.reshape(BT,C,F)
...
class MappingNet(nn.Module):
    ...
    def forward(self, X, tgru_state_in1, tgru_state_in2, tgru_state_in3):

        X = self.m_in(X)
        B, F, T, C = X.shape
        X = self.encoder(X.reshape(B*F,T,C).permute(0,2,1)).permute(0,2,1).reshape(B,F,1,-1)
        ...
        return Y, tgru_state_out1, tgru_state_out2, tgru_state_out3
    ...
    def to_onnx(self, output_fp, device=torch.device("cpu")):
        ...
        dummy_input = torch.randn(1,self.n_rfft, 1, 6).to(device)
        dummy_states1, _  = self.helper.create_dummy_states(1,device)
        dummy_states2, _  = self.helper.create_dummy_states(1,device)
        dummy_states3, _  = self.helper.create_dummy_states(1,device)

        torch.onnx.export(
            self,
            (dummy_input, dummy_states1, dummy_states2, dummy_states3),
            output_fp,
            verbose=False,
            opset_version=16,
            input_names=["inputs", "h0", "h1", "h2"],
            output_names=["outputs", "h0_out", "h1_out", "h2_out"],
        )
