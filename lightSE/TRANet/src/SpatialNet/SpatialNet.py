...
class SpatialNet(nn.Module):
    ...
    def __init__(
        ...
        self.frame_size = (num_freqs-1)*2
        self.hop_size = num_freqs-1
    ...
    def _to_spec(self,x):
        ...
        if self.frame_size == self.hop_size :
            ...
            X = torch.stft(x, n_fft = self.frame_size, hop_length=self.hop_size ,window=torch.hann_window(self.frame_size).to(x.device),return_complex=False)
    ...
    def forward(self, x: Tensor, return_attn_score: bool = False) -> Tensor:
        ...
        Y = self.decoder(x)
        _,_,_,p = Y.shape
        y = [self._to_signal(Y[:,:,:,i*p//2:(i+1)*p//2])[:,:L].contiguous() for i in range(p//2)]
        if return_attn_score:
            return y, attns
        else:
            return y
