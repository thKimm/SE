...
class DatasetDNS(torch.utils.data.Dataset):
    def __init__(self,hp,is_train=True):
        ...
        if is_train : 
            ...
            self.list_noise = glob(join(hp.data.dev.noise,"**","*.wav"),recursive=True)
            self.list_RIR   = glob(join(hp.data.dev.RIR,"**","*.wav"),recursive=True)
        ...
        if is_train : 
            print("DatasetDNS[train:{}] | len : {} | clean {} | noise : {} | RIR : {}".format(is_train,len(self.list_clean),len(self.list_clean),len(self.list_noise),len(self.list_RIR)))
        else :
            print("DatasetDNS[train:{}] | len : {} | clean {} | noise : {} | RIR : {}".format(is_train,len(self.list_clean),len(self.list_clean),len(self.list_noise),len(self.list_RIR)))
    ...
    def mix(self,clean,noise,rir,noise_rir=None,eps=1e-7):
        if self.hp.task == "LGHnA":
            if rir is not None:
                clean_ = np.repeat(signal.fftconvolve(clean, rir[0,:][:-64])[:,np.newaxis],3,axis=1).T
                for i in range(3):
                    clean_[i,:] = signal.fftconvolve(clean, rir[i,:][:-64])
                clean = clean_
